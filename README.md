## Привет 👋 Я RuRen1 
Я являюсь студентом Национального исследовательского университета ИТМО.

## В данный момент я пользуюсь Гитом для сдачи лаборатоных по таким предметам, как:
1) Архитектуры вычислительных систем
2) Алгоритмы и структуры данных
3) Программирование и обработка данных на Python

## Языки и инструменты
### Языки:
Python
C
### Инструменты: 
Git 
Linux

## 📈 GitHub статистика
![GitHub stats](https://github-readme-stats.vercel.app/api?username=RuRen1&show_icons=true&theme=default)

## 📫 Контакты
- GitHub: [github.com/RuRen1](https://github.com/RuRen1)
- Telegram: @grigorii_tygosera
<!--
**RuRen1/RuRen1** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
